...
const evidence: { name: string; tone: "ok" | "warn"; state: string; note: string }[] = [
  ...
  },
  {
    ...
    tone: "warn",
    state: "dedicated devnet provider configured at source; live reading still not proven",
    note: "A reviewed request-only devnet reader now exists and never creates the wallet SDK connection object that this runtime cannot build. It checks the network fingerprint, takes a settled block reference and confirms the network charge stays inside the reserved allowance. The failed live check was diagnosed in an isolated throwaway runtime, not guessed: this host refuses one of the request's safety settings outright, before anything is sent, which is why no reply status was ever seen. That setting has been replaced with an equivalent that keeps the same rule — redirected requests are still never followed — and the failure reporting now recognises a fixed list of known causes without ever carrying raw error text. This is a source fix only; the live check is still switched off and live network reading remains NOT proven. Nothing is wired to wallets or the chat bot, no send path is reachable, and no value may be substituted from stored records. The reader now uses the owner's dedicated devnet provider, assembled inside the service from the stored devnet credential; an optional explicit address is accepted only when it is the same secure devnet provider, and anything else — including the mainnet address — is refused with no fallback. The mainnet credential is never read.",
  },

  {
    name: "Diagnostic probe",
    tone: "warn",
    state: "switched off again after the bounded check; live pending publish",
    note: "The switch was turned on for a single reviewed read-only network check and turned off again immediately after the evidence was captured. It is off at source; the live service keeps the previous setting until the next publish. No caller credential was ever exposed.",
  },
  {
